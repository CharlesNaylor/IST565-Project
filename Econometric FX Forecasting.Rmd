...
Spot rates, at least should be freely available. However, once again, data from Yahoo turned out to be poor enough that proprietary Bloomberg data was needed. Supposedly weekly data from Yahoo included some data marked from all days of the week, although not all with the same frequency.
...
env_eqy <- new.env() #quantmod likes to drop new variables directly into the root environmen
...
```{r tf_helper_functions}
library(tensorflow)

esn_model_fn <- function(features, labels, mode, params, config) {
  #construct a few ESN layers, handle parameter tuning.
  # We have scaled inputs already, so we just need to worry about
  # spectral radius and leaking rates.
  # the spectral radius must be < 1 for the RNN layer to be considered an ESN.
  
  #input layer: we have 2D data consisting of the residuals for each asset each week. We might batch the data if we choose to use HMC-generated forecasts.
  input_layer = tf$reshape(features$x, c(-1L, 505L, 11L))
  size = 100
