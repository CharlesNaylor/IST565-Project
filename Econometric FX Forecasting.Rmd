...
data {
  ...
  matrix[T,A] Y; // asset returns
  matrix[T,J] X[A]; //exogenous panel data
...
transformed data {
  ...
  matrix[A,A] L_y_cov[TL];
  TL = T_1 - T;
  for(t in T_1:T) {
    L_y_cov[t-T_1+1] = cholesky_decompose(y_cov[t]); // Cholesky decompositions are more stably positive-definite than straight covariance matrices
  }
}
parameters {
...
model {
  ...
  for(t in 1:T) { //for each week in the data
      for(i in 1:t) { //for each period prior to that week
        target += multinormal_cholesky_lpdf(to_row_vector(Y[t])|XB[t], L_y_cov[t]);
      }
    }
  }
}
